...
function loadCheerio(htmlBody) {
    ...
    $.prototype.hasNoStrain = function () {
        let result = false;
        this.each(function () {
            if ($(this).attr('no-strain') !== undefined) {
                result = true;
            }
        });
        return result;
            ...
            this.each(function () {
                ...
                if (prefix === '#') {
                    if (!$(this).hasNoStrain()) {
                        ...
                        const label_for_query = `[for="${old_property_name}"]`;
                        $(label_for_query).attr('for', new_property_name);
                    }
                } else if (prefix === '.') {
                    const new_class_data = `[${old_property_name}]`;
                    const current_class_data = $(this).getStrClassData();
                    if (!current_class_data.includes(new_class_data) && !$(this).hasNoStrain()) {
